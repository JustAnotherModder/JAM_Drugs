...
function JAM_Drugs:FindNearestMarker(pos) 
    ...
    for k,v in pairs(self.Config.Zones) do 
        local distZone = self:GetVecDist(pos, v.ZonePos.xyz)
        local distExit = self:GetVecDist(pos, v.ExitPos.xyz)
        local distAction = self:GetVecDist(pos, v.ActionPos)
        local distSafe
        if v.SafeActionPos then distSafe = self:GetVecDist(pos, v.SafeActionPos); end

        if not nearestDist or nearestDist > distZone then
            nearest,nearestDist,nearestCoords = v,distZone,v.ZonePos.xyz 
        end

        if not nearestDist or nearestDist > distExit then 
            nearest,nearestDist,nearestCoords = v,distExit,v.ExitPos.xyz
        end

        if not nearestDist or nearestDist > distAction then 
            nearest,nearestDist,nearestCoords = v,distAction,v.ActionPos
        end

        if distSafe then 
          if not nearestDist or nearestDist > distSafe then 
